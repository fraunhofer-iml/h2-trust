/*
 * Copyright Fraunhofer Institute for Material Flow and Logistics
 *
 * Licensed under the Apache License, Version 2.0 (the "License").
 * For details on the licensing terms, see the LICENSE file.
 * SPDX-License-Identifier: Apache-2.0
 */

import { Unit } from '@prisma/client';
import { UnitType } from '@h2-trust/domain';
import { AddressSeed } from '../address.seed';
import { auditTimestamp } from '../audit-timestamp.constant';
import { CompanySeed } from '../company.seed';

export const UnitSeed: readonly Unit[] = Object.freeze([
  {
    id: 'power-production-unit-0',
    createdAt: auditTimestamp,
    updatedAt: auditTimestamp,
    type: UnitType.POWER_PRODUCTION,
    name: 'Photovoltaic System 001',
    manufacturer: 'AgriVolt Systems',
    modelType: 'EcoGen1500',
    modelNumber: 'EG1500-BIO',
    serialNumber: 'PPU-SN-0002',
    certifiedBy: 'TÜV NORD AG',
    commissionedOn: new Date('2023-06-15'),
    addressId: AddressSeed[2].id,
    ownerId: CompanySeed[2].id,
    operatorId: CompanySeed[2].id,
    active: false,
    specificationId: 'power-production-unit-0',
  },
  {
    id: 'power-production-unit-1',
    createdAt: auditTimestamp,
    updatedAt: auditTimestamp,
    type: UnitType.POWER_PRODUCTION,
    name: 'Wind Turbine 001',
    manufacturer: 'Wind Energy GmbH',
    modelType: 'WE 4.5/75',
    modelNumber: 'WE-4.5/75-V5',
    serialNumber: 'WE4575-002561',
    certifiedBy: 'EnergieControl Zert GmbH',
    commissionedOn: new Date('2025-03-19'),
    addressId: AddressSeed[2].id,
    ownerId: CompanySeed[2].id,
    operatorId: CompanySeed[2].id,
    active: true,
    specificationId: 'power-production-unit-1',
  },
  {
    id: 'power-production-unit-2',
    createdAt: auditTimestamp,
    updatedAt: auditTimestamp,
    type: UnitType.POWER_PRODUCTION,
    name: 'Wind Turbine 002',
    manufacturer: 'Wind Energy GmbH',
    modelType: 'WE 4.5/75',
    modelNumber: 'WE-4.5/75-V5',
    serialNumber: 'WE4575-002591',
    certifiedBy: 'EnergieControl Zert GmbH',
    commissionedOn: new Date('2025-03-19'),
    addressId: AddressSeed[4].id,
    ownerId: CompanySeed[0].id,
    operatorId: CompanySeed[0].id,
    active: true,
    specificationId: 'power-production-unit-2',
  },
  {
    id: 'power-production-unit-3',
    createdAt: auditTimestamp,
    updatedAt: auditTimestamp,
    type: UnitType.POWER_PRODUCTION,
    name: 'Grid Connection',
    manufacturer: CompanySeed[3].name,
    modelType: 'GridConnection',
    modelNumber: 'GRID-001',
    serialNumber: 'GRID-SN-0001',
    certifiedBy: 'TÜV Saarland e. V.',
    commissionedOn: new Date('2025-08-14'),
    addressId: AddressSeed[1].id,
    ownerId: CompanySeed[1].id,
    operatorId: CompanySeed[1].id,
    active: true,
    specificationId: 'power-production-unit-3',
  },
  {
    id: 'hydrogen-production-unit-0',
    createdAt: auditTimestamp,
    updatedAt: auditTimestamp,
    type: UnitType.HYDROGEN_PRODUCTION,
    name: 'Hydrogen Electrolyzer Dortmund 001',
    manufacturer: 'HydroNova Technologies GmbH',
    modelType: 'HN-Elektrolyser Pro X',
    modelNumber: 'HN-EL-PX500',
    serialNumber: 'PX500-00034872',
    certifiedBy: 'EnergieControl Zert GmbH',
    commissionedOn: new Date('2025-03-15'),
    addressId: AddressSeed[5].id,
    ownerId: CompanySeed[2].id,
    operatorId: CompanySeed[2].id,
    active: true,
    specificationId: 'hydrogen-production-unit-0',
  },
  {
    id: 'hydrogen-storage-unit-0',
    createdAt: auditTimestamp,
    updatedAt: auditTimestamp,
    type: UnitType.HYDROGEN_STORAGE,
    name: 'Hydrogen Storage Dortmund 001',
    manufacturer: 'HydroNova Technologies GmbH',
    modelType: 'HN-Storage X',
    modelNumber: 'HN-STO-X50',
    serialNumber: 'STO50-0000527',
    certifiedBy: 'EnergieControl Zert GmbH',
    commissionedOn: new Date('2025-03-18'),
    addressId: AddressSeed[5].id,
    ownerId: CompanySeed[2].id,
    operatorId: CompanySeed[2].id,
    active: true,
    specificationId: 'hydrogen-storage-unit-0',
  },
  {
    id: 'hydrogen-bottling-unit-0',
    createdAt: auditTimestamp,
    updatedAt: auditTimestamp,
    type: UnitType.BOTTLING,
    name: 'Hydrogen Bottling Unit Dortmund 001',
    manufacturer: 'HydroNova Technologies GmbH',
    modelType: 'HN-Storage X',
    modelNumber: 'HN-STO-X50',
    serialNumber: 'STO50-0000528',
    certifiedBy: 'EnergieControl Zert GmbH',
    commissionedOn: new Date('2025-03-18'),
    addressId: AddressSeed[5].id,
    ownerId: CompanySeed[2].id,
    operatorId: CompanySeed[2].id,
    active: true,
    specificationId: 'hydrogen-bottling-unit-0',
  },
  {
    id: 'hydrogen-compression-unit-0',
    createdAt: auditTimestamp,
    updatedAt: auditTimestamp,
    type: UnitType.COMPRESSION,
    name: 'Hydrogen Compression Unit Dortmund 001',
    manufacturer: 'HydroNova Technologies GmbH',
    modelType: 'HN-Storage X',
    modelNumber: 'HN-STO-X50',
    serialNumber: 'STO50-0000529',
    certifiedBy: 'EnergieControl Zert GmbH',
    commissionedOn: new Date('2025-03-18'),
    addressId: AddressSeed[5].id,
    ownerId: CompanySeed[2].id,
    operatorId: CompanySeed[2].id,
    active: true,
    specificationId: 'hydrogen-compression-unit-0',
  },
  {
    id: 'hydrogen-end-use-unit-0',
    createdAt: auditTimestamp,
    updatedAt: auditTimestamp,
    type: UnitType.END_USE,
    name: 'Hydrogen End Use Unit Dortmund 001',
    manufacturer: 'HydroNova Technologies GmbH',
    modelType: 'HN-Storage X',
    modelNumber: 'HN-STO-X50',
    serialNumber: 'STO50-0000530',
    certifiedBy: 'EnergieControl Zert GmbH',
    commissionedOn: new Date('2025-03-18'),
    addressId: AddressSeed[5].id,
    ownerId: CompanySeed[2].id,
    operatorId: CompanySeed[2].id,
    active: true,
    specificationId: 'hydrogen-end-use-unit-0',
  },
  {
    id: 'hydrogen-transport-unit-0',
    createdAt: auditTimestamp,
    updatedAt: auditTimestamp,
    type: UnitType.TRANSPORTATION,
    name: 'Hydrogen Transport Unit 001',
    manufacturer: 'Transport Unit Manufacturer',
    modelType: 'Transport Unit Model Type',
    modelNumber: 'Transport Unit Model Number',
    serialNumber: 'Transport Unit Model Serial Number',
    certifiedBy: 'Transport Unit Certifier',
    commissionedOn: new Date('2025-03-18'),
    addressId: AddressSeed[5].id,
    ownerId: CompanySeed[2].id,
    operatorId: CompanySeed[2].id,
    active: true,
    specificationId: 'hydrogen-transport-unit-0',
  },
]);
