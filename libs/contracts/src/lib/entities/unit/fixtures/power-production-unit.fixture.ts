...
export const PowerProductionUnitEntityFixture = {
  create: (overrides: Partial<UnitEntity> = {}): UnitEntity =>
    ({
      ...
      decommissioningPlannedOn: overrides.details?.decommissioningPlannedOn ?? new Date('2025-01-01'),
      ratedPower: overrides.details?.ratedPower ?? 1000,
      biddingZone: overrides.details?.biddingZone ?? BiddingZone.DE_LU,
      financialSupportReceived: overrides.details?.financialSupportReceived ?? false,
      active: true,
