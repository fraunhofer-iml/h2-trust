  ...
  describe('storeProof', function () {
    ...
    describe(FAILURE, function () {
      it('should revert when called by non-owner', async function () {
        ...
        await expect(bobProofStorage.storeProof(UUID, HASH, CID)).to.be.revertedWithCustomError(
          proofStorage,
          'NotOwner',
        );
      });
      ...
      it('should revert when hash is empty', async function () {
        ...
        await expect(proofStorage.storeProof(UUID, '', CID)).to.be.revertedWithCustomError(proofStorage, 'HashEmpty');
      });

      it('should revert when cid is empty', async function () {
        const { proofStorage } = await deployProofStorage();

        await expect(proofStorage.storeProof(UUID, HASH, '')).to.be.revertedWithCustomError(proofStorage, 'CidEmpty');
      });
    });
  });
