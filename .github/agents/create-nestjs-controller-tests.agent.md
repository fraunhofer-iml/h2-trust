...
- Assert transformed payloads and boundary interactions with focused expectations or `expect.objectContaining(...)` when
  full equality would be brittle.
...
- Use TestingModule only when Nest dependency injection behavior matters; keep helper logic outside the Nest container
  when framework wiring is not the thing under test.
