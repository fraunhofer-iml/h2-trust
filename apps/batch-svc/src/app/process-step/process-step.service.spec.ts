...
describe('ProcessStepService', () => {
  ...
  describe('readProcessSteps', () => {
    it('should return process steps for given criteria', async () => {
      ...
      // Act
      const actualResponse = await service.readProcessSteps(givenProcessTypes, givenPredecessorProcessTypes, givenActive, givenCompanyId);

      // Assert
      expect(repository.findProcessSteps).toHaveBeenCalledWith(givenProcessTypes, givenPredecessorProcessTypes, givenActive, givenCompanyId);
  ...
  describe('readProcessStep (HYDROGEN_TRANSPORTATION)', () => {
    it('should use predecessor documents for transportation steps', async () => {
      ...
      hydrogenTransportationFixture.batch.predecessors = [
        {
          processStepId: hydrogenBottlingFixture.id,
        },
      ];
    ...
    it('throws when predecessor type is not HYDROGEN_BOTTLING', async () => {
      ...
      hydrogenTransportationFixture.batch.predecessors = [
        {
          processStepId: hydrogenProductionFixture.id,
        },
      ];
