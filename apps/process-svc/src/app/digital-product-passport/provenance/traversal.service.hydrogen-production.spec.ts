...
describe('TraversalService', () => {
  ...
  describe('fetchHydrogenBottlingFromHydrogenTransportation', () => {
    it(`throws error when process step is not ${ProcessType.HYDROGEN_TRANSPORTATION} type`, async () => {
      ...
      // Act & Assert
      await expect(service.fetchHydrogenBottlingFromHydrogenTransportation(givenWrongProcessStep)).rejects.toThrow(
    ...
    it('throws error when hydrogenTransportation has no predecessors', async () => {
      ...
      // Act & Assert
      await expect(
        service.fetchHydrogenBottlingFromHydrogenTransportation(givenHydrogenTransportation),
    ...
    it('throws error when more than one hydrogen bottling predecessor is found', async () => {
      ...
      const givenHydrogenBatch1 = BatchEntityFixture.createHydrogenBatch({
        id: 'batch-1',
        processStepId: givenHydrogenBottling1.id,
      });
      const givenHydrogenBatch2 = BatchEntityFixture.createHydrogenBatch({
        id: 'batch-2',
        processStepId: givenHydrogenBottling2.id,
      });
