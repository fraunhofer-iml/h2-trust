...
describe('TraversalService', () => {
  ...
  describe('fetchPowerProductionsFromHydrogenProductions', () => {
    it('throws error when hydrogenProductions is empty', async () => {
      ...
      // Act & Assert
      await expect(service.fetchPowerProductionsFromHydrogenProductions(givenHydrogenProductions))
    ...
    it('throws error when hydrogenProduction has no predecessors', async () => {
      ...
      // Act & Assert
      await expect(service.fetchPowerProductionsFromHydrogenProductions([givenHydrogenProduction]))
        .rejects.toThrow(expectedErrorMessage);
    });
    ...
    it('returns multiple power production process steps from multiple hydrogen productions', async () => {
      ...
      const givenPowerBatch1 = BatchEntityFixture.createPowerBatch({ id: 'batch-1', processStepId: givenPowerProduction1.id });
      const givenPowerBatch2 = BatchEntityFixture.createPowerBatch({ id: 'batch-2', processStepId: givenPowerProduction2.id });
