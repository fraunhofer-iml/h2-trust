 ...
 */
export function normalizeProduction(parsedImports: ParsedImport[], ownerId: string): StagedProductionEntity[] {
  const parsedAccountingPeriodGroups: Map<CsvContentType, UnitAccountingPeriods[]> =
    groupAccountingPeriodsByType(parsedImports);
  ...
  for (const [productionType, parsedAccountingPeriodGroup] of parsedAccountingPeriodGroups) {
    const stagedProductionsForType: StagedProductionEntity[] = normalizeAccountingPeriods(
...
function groupAccountingPeriodsByType(parsedImports: ParsedImport[]): Map<CsvContentType, UnitAccountingPeriods[]> {
...
function normalizeAccountingPeriods(
  ...
  unitAccountingPeriods.forEach((unitAccountingPeriod) => {
    ...
    Object.entries(hourlyProductionTotals).forEach(([timestamp, amount]) => {
      addToMap<StagedProductionEntity>(unitAccountingPeriodsByDateHour, `${timestamp}:00:00Z`, {
...
function addToMap<T extends PowerItem | HydrogenItem>(map: Map<string, T[]>, key: string, value: T): void {
  if (!map.get(key)) {
    map.set(key, [value]);
  } else {
