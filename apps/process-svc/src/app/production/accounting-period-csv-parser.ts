...
import { BrokerException, StageProductionAccountingPeriod } from '@h2-trust/amqp';
...
export class AccountingPeriodCsvParser {
  ...
  static async parseBuffer(buffer: Buffer, columns: string[]): Promise<StageProductionAccountingPeriod[]> {
    let skipped = 0;
    let invalid = 0;
    ...
    return new Promise((resolve, reject) => {
      const records: StageProductionAccountingPeriod[] = [];

      parser.on('readable', () => {
        let record: StageProductionAccountingPeriod | null;
