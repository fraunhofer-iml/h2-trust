...
describe('TraversalService', () => {
  ...
  describe('fetchPowerProductionsFromHydrogenProductions', () => {
    it(`throws if ${ProcessType.HYDROGEN_PRODUCTION} process steps are empty`, async () => {
      ...
      // act & assert
      await expect(service.fetchPowerProductionsFromHydrogenProductions(givenProcessSteps)).rejects.toThrow(
        expectedError,
      );
    });
    ...
    it(`should return one ${ProcessType.POWER_PRODUCTION} process step`, async () => {
      ...
      const expectedResult: ProcessStepEntity[] = [createProcessStep('pp1', ProcessType.POWER_PRODUCTION, [])];

      batchSvcSendMock.mockReturnValue(of(expectedResult.at(0)));

      // act
      const actualResult: ProcessStepEntity[] =
        await service.fetchPowerProductionsFromHydrogenProductions(givenProcessSteps);
