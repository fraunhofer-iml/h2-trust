...
export class ProductionService {
  ...
  async importCsvFiles(
    ...
    const payload = new StageProductionsPayload(stageProductions, userId, userDetails.company.id, dto.timeZoneName);
  ...
  async readCsvDocumentsByCompany(userId: string): Promise<ProcessedCsvDto[]> {
    ...
    return csvDocuments.map((doc) =>
      ProcessedCsvDto.fromEntity(doc, this.storageService.endpointUrl, userDetails.company.name),
    );
  }
