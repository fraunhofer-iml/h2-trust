...
export class RedCompliancePairingService {
  ...
  async buildMatchedPairs(
    ...
    const pairs: MatchedProductionPair[] = this.buildMatchedPairsWithoutUnits(
      powerProductions,
      hydrogenProductions,
    );
  ...
  private buildMatchedPairsWithoutUnits(
    ...
    const hydrogenProductionById = this.buildHydrogenProductionById(
      hydrogenProductions,
    );
    const pairs: MatchedProductionPair[] = [];
    for (const powerProduction of powerProductions) {
      const successor = powerProduction.batch?.successors?.find((s) =>
        hydrogenProductionById.has(s.processStepId),
      );
      if (successor) {
        const hydrogenProduction = hydrogenProductionById.get(
          successor.processStepId,
        )!;
        pairs.push({
  ...
  private buildHydrogenProductionById(
    hydrogenProductions: ProcessStepEntity[],
  ): Map<string, ProcessStepEntity> {
  ...
  private async enrichPairsWithUnits(
    ...
    const { uniquePowerUnitIds, uniqueHydrogenUnitIds } =
      this.collectUniqueUnitIdsFromPairs(pairs);
  ...
  private collectUniqueUnitIdsFromPairs(pairs: MatchedProductionPair[]): {
    ...
    const uniquePowerUnitIds = Array.from(
      new Set(
        pairs.map((pair) => pair.power.processStep.executedBy.id),
      ),
    );
    const uniqueHydrogenUnitIds = Array.from(
      new Set(
        pairs.map((pair) => pair.hydrogen.processStep.executedBy.id),
      ),
    );
  ...
  private async loadProductionUnits(
    ...
    const [powerProductionUnitEntities, hydrogenProductionUnitEntities] =
      await Promise.all([
        firstValueFrom(
    ...
    for (const unit of powerProductionUnitEntities)
      powerUnitById.set(unit.id, unit);
    const hydrogenUnitById = new Map<string, HydrogenProductionUnitEntity>();
    for (const unit of hydrogenProductionUnitEntities)
      hydrogenUnitById.set(unit.id, unit);
